...
//                        System.out.println(t.toString());
    ...
    /*MÉTODO QUE DEVUELVA UN MAP, DONDE LA 'KEY' ES EL ID DE LOS PRODUCTOS QUE DESEA 
    PEDIR EL USUARIO Y 'VALUE' SEA LA CANTIDAD DE ESE PRODUCTO , SI EL USUARIO 
    VUELVE A PEDIR ESE PRODUCTO ESTE VALUE SE INCREMENTA*/
    public static Map<Integer, Integer> carritoMAP(int idProductoDeseado/*, int cantidadProducto*/, ArrayList<Producto> listaProductos) {
        Map<Integer, Integer> mapProductos = new HashMap<>();
        //int cantidadProducto = 9999;

        // Recorro la lista de productos
        for (int i = 0; i < listaProductos.size(); i++) {
            if (listaProductos.get(i).getID() == idProductoDeseado) {
                int cantidadProducto = preguntarCantidadGeneral();
                modificarStock(listaProductos, idProductoDeseado);
                if (listaProductos.get(i).getStock() > 0 && cantidadProducto <= listaProductos.get(i).getStock()) {
                    // Si el producto deseado ya esta en el Map solo cambio su value
                    // value = anteriorValue+ cantProd
                    if (mapProductos.containsKey(listaProductos.get(i))) {
                        int cantidadPrevia = mapProductos.get(listaProductos.get(i).getID());
                        //mapProductos.put(listaProductos.get(i).getID(), (cantidadPrevia + cantidadProducto));
                        mapProductos.put(idProductoDeseado, cantidadPrevia + cantidadProducto);

                    } else {
                        // Si no está en la lista, meto su id y la cantidad tal cual.
                        mapProductos.put(idProductoDeseado, cantidadProducto);
                    }
                }
            }
        }
    ...
    public static void mostrarMap(Map<Integer, Integer> mapProductos, ArrayList<Producto> listaProductos) {
        System.out.println("*************************************************************************");
        for (Map.Entry<Integer, Integer> entry : mapProductos.entrySet()) {

            for (Producto listaProducto : listaProductos) {
                if (listaProducto.getID() == entry.getKey()) {
                    System.out.println(entry.getValue() + " - " + listaProducto.getDescripción());
                }
